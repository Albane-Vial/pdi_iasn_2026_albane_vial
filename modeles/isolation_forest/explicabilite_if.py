...
def _pipeline_prediction_boite_noire(textes, modele_if, pca_model, scaler_model, tokenizer, bert_model):
    """
    Fonction interne évaluant un tableau de textes et retournant le score d'anomalie 
    minimal (le plus critique) pour chaque phrase.
    """
...
def generer_explicabilite_lime_if(df_audit: pd.DataFrame, modele_if, pca_model, scaler_model, tokenizer, bert_model, chemin_export: str):
    """
    Génère les explications LIME en convertissant les distances du graphe d'isolement 
    en pseudo-probabilités logistiques.
    """
...
def extraire_erreurs_pour_explicabilite(chemin_predictions_parquet: str):
    ...
    # 3. Extraire un exemple par type d'erreur pour les FP
    # On utilise dropna() pour s'assurer que l'exemple a bien un type d'erreur défini
    exemples_fp = df_fp.groupby('error_types').first().reset_index()
    
    # 4. Extraire un exemple par type d'erreur pour les FN
