...
DATA_PATH = '/kaggle/input/mimic-iv-2-1/mimic-iv-2.1/hosp/'
...
def extraire_donnée_precriptions():
    ...
    df_traitement['q01'] = df_traitement['drug'].map(quantiles['q01'])
    df_traitement['q99'] = df_traitement['drug'].map(quantiles['q99'])

    df_traitement = df_traitement[
        (df_traitement['dose_val_rx'] >= df_traitement['q01']) & 
        (df_traitement['dose_val_rx'] <= df_traitement['q99'])
    ]
